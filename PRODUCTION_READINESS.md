...
- `web/platho-config.mjs` must be pinned to the verified mainnet manifest and `pwa-production` signed-bundle purpose before publishing the production PWA. `MAINNET_GENESIS_VERIFIED.txt` returns `false` because the clean-12 genesis is not yet deployed, sealed, or verified, so the current archive may still be preview-blocked until the genesis is verified; the config is pinned to the clean-12 draft mainnet manifest hash `ff26bda7751829e0450a9665fc2f4a619c9f9e4a7d659d45a6728deef11ece95` and its Vault, CapsuleHub, ATHMaster, UsernameRegistry, ProfileRegistry, and `deploymentManifestHash` will be re-confirmed against the verified mainnet manifest at seal. Treat `artifacts/mainnet_genesis_verify_report.json`, `artifacts/MAINNET_GENESIS_VERIFIED.txt`, and `preprod:check` as the source of truth.
