...
- `web/platho-config.mjs` must be pinned to the verified mainnet ATHMaster, UsernameRegistry, ProfileRegistry, FeeAccumulator, AirdropPool, manifest hash, and production signed-bundle purpose before publishing the production PWA bundle. [CORRECTED 2026-07-31 - this demanded pins to `Vault` and `CapsuleHub`, both DELETED in clean-17; an operator could not have satisfied it.]
- Production TON RPC configuration must use approved mainnet providers. Critical reads stay fail-closed on disagreement or missing verification while a primary transport is healthy, and shard entry reads (RecordShard / IntroShard / PublicShard) must have at least one concrete provider with account-state and message-history support.
- Censorship survival is a release requirement: the production config must include a keyless verifier-only emergency fallback provider (direct TonCenter) with concrete read, send, and message-history endpoints (`PWA_TON_RPC_EMERGENCY_FALLBACK_REQUIRED`). If the Platho RPC gateway is blocked or unreachable, the transport parks it after repeated hard failures, promotes the emergency provider to full duty at its ~1 rps budget, and re-probes the gateway on a retry window. In that degraded single-provider mode dual-provider verification is structurally impossible, so own-action pre-sign reads may fall back to unverified reads through `callWithDegradedTransportReadFallback`; trust-critical recipient key verification gates remain fail-closed. Post-broadcast nonce polling is always unverified and tolerant because the publish outcome is re-authenticated by reading the shard entry itself. The TonCenter API key never ships in the static bundle; the direct emergency endpoints are keyless by design.
...
- The one-shot `ATHMaster.DeployTreasurySupply` transaction (ceremony step D02) is COMPLETE for clean-17. It minted the entire supply into the treasury owner's ATH wallet, which holds `100,000,000 ATH`. It is one-shot: do not attempt it again. [CORRECTED 2026-08-07 - between 2026-07-31 and this line the clean-17 genesis was performed and sealed (2026-08-02), and the 2026-07-31 correction that said "has NOT been performed" was left behind. A stale "not performed" is the mirror of the defect the 2026-07-31 pass fixed: it tells the operator to run an IRREVERSIBLE one-shot that already ran. Verified against `MAINNET_GENESIS_VERIFIED.txt` = `true` and a live read of the pool's ATH wallet, which could not hold ATH if the mint had not run.]
- AirdropPool activity airdrop backing is COMPLETE for clean-17: the official AirdropPool ATH wallet was funded with `15,000,000 ATH` and the live verifier proved that backing. Publish rewards are live and paying — a live read on 2026-08-07 showed `14,997,820 ATH` remaining, so `2,180 ATH` had already been delivered. [CORRECTED 2026-08-07 - same stale-correction defect as the line above; this still said the funding "has NOT been performed".]
...
-> 15,000,000 ATH activity airdrop distribution through AirdropPool
-> AirdropPool remaining_budget == 0 proof
...
- `web/app.js` uses a normal 24-word TON recovery phrase as the single user secret, derives wallet and messaging keys from that phrase, and has a fail-closed chain binding bridge to the user's own KeyShard.
- The PWA anchors public messaging keys in the wallet's own KeyShard account and publishes private messages straight into RecordShard (CONV) and IntroShard (first contact) by direct pay. [CORRECTED 2026-07-31 - described the DELETED Vault/CapsuleHub design.] Manual public-bundle / capsule package exchange is removed from the production UI.
...
- `web/platho-config.mjs` must be pinned to the verified mainnet manifest and `pwa-production` signed-bundle purpose before publishing the production PWA. `MAINNET_GENESIS_VERIFIED.txt` returns `true`: the clean-17 genesis was deployed and sealed on 2026-08-02, and the live verifier report carries `MAINNET_GENESIS_VERIFIED` with no issue codes against manifest `ccee504c6b94773e68550dff3a071107f34609b0fdb17be9fad8ab224d482f49`. The clean-15 record was deliberately retired rather than carried forward, because clean-17 differs from it in every one of its manifest code hashes. Treat `artifacts/mainnet_genesis_verify_report.json`, `artifacts/MAINNET_GENESIS_VERIFIED.txt`, and `preprod:check` as the source of truth; the manifest hash to pin is the one the ceremony emitted, never a hash copied from a previous generation. [CORRECTED 2026-08-07 - this said the flag returns `false`, that the genesis "has not been deployed", and that the production bundle reports the blocker `MAINNET_GENESIS_NOT_VERIFIED`. All three were true on 2026-07-28 and false since 2026-08-02; the production package reports `PRODUCTION_STATIC_PACKAGE_READY` with an empty blocker list.]
...
- A shard entry is the message. RecordShard, IntroShard and PublicShard persist the authenticated commit and the reader recovers the body from the publish transaction itself [CORRECTED 2026-07-31 - this described CapsuleHub v1, DELETED in clean-17]; the old wording spoke of compact authenticated entry state (`body_hash`, headers, `created_at`, and indexes). A state record without the matching accepted publish transaction body is not a readable v1 message; a counter-only, anchor-only, off-chain pointer, or local-cache-only message record is not valid.
- Storage top-up ABI coverage is explicit for ATHWallet, ATHMaster, ATHVesting, FeeAccumulator, BuybackBurn, MarketStabilitySeller, AirdropPool, UsernameRegistry, ProfileRegistry, KeyShard and UsernameNFTItem. Top-up messages are no-authority storage reserve maintenance only, not balances or rescue paths.
