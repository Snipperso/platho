...
- `web/platho-config.mjs` must be pinned to the verified mainnet manifest and `pwa-production` signed-bundle purpose before publishing the production PWA. `MAINNET_GENESIS_VERIFIED.txt` returns `true` because the clean-15 genesis (username NFT reference TEP-62 transfer: explicit-rest excesses, addr_none response_destination, dynamic forward-fee allowance) is deployed, sealed, and verified on mainnet; the config is pinned now and must be pinned to the verified mainnet manifest hash `d9ca407acd7a9cdb5b3ee26cbd122b73fea79e6f7f12fb7445c1a64516e57a16`, and its Vault, CapsuleHub, ATHMaster, UsernameRegistry, ProfileRegistry, and `deploymentManifestHash` match the verified mainnet manifest confirmed at seal (the 60M MarketStabilitySeller reserve is capitalized as part of the certified final genesis). Treat `artifacts/mainnet_genesis_verify_report.json`, `artifacts/MAINNET_GENESIS_VERIFIED.txt`, and `preprod:check` as the source of truth.
