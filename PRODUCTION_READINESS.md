...
- `web/platho-config.mjs` must be pinned to the verified mainnet manifest and `pwa-production` signed-bundle purpose before publishing the production PWA. `MAINNET_GENESIS_VERIFIED.txt` returns `false` while the clean-15 genesis (username NFT reference TEP-62 transfer: explicit-rest excesses, addr_none response_destination, dynamic forward-fee allowance) is regenerated locally but not yet deployed, sealed, or verified on mainnet; until `mainnet:genesis:verify` passes against a fresh clean-15 live snapshot the current archive may still be preview-blocked and must not be published to production. The config still carries the live clean-14 addresses; it will be re-pinned to the clean-15 manifest hash `d9ca407acd7a9cdb5b3ee26cbd122b73fea79e6f7f12fb7445c1a64516e57a16` (with its Vault, CapsuleHub, ATHMaster, UsernameRegistry, ProfileRegistry, and `deploymentManifestHash`) only after the clean-15 seal is verified. Treat `artifacts/mainnet_genesis_verify_report.json`, `artifacts/MAINNET_GENESIS_VERIFIED.txt`, and `preprod:check` as the source of truth.
...
- `market-stability:readiness` is not a substitute for `mainnet:genesis:verify`. The release order is `mainnet:genesis:verify` PASS, then production PWA release, then full `15,000,000 ATH` activity airdrop distribution, then initial pool launch, then post-pool pricing freeze, then reserve funding, then `market-stability:readiness` PASS. Do not run the seller readiness script as a standalone production gate.
