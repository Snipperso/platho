...
- `web/platho-config.mjs` must be pinned to the verified mainnet manifest and `pwa-production` signed-bundle purpose before publishing the production PWA. `MAINNET_GENESIS_VERIFIED.txt` returns `false` while the clean-14 genesis (F11 hardening: BuybackBurn dead-man, ATHWallet storage top-up, MarketStabilitySeller floor) is regenerated locally but not yet deployed, sealed, or verified on mainnet; until `mainnet:genesis:verify` passes against a fresh clean-14 live snapshot the current archive may still be preview-blocked and must not be published to production. The config still carries the live clean-13 addresses; it will be re-pinned to the clean-14 manifest hash `9e82eaa3f96d533a99a397cd71713736fda2e49cfdf84b495353d0bfda91f8ad` (with its Vault, CapsuleHub, ATHMaster, UsernameRegistry, ProfileRegistry, and `deploymentManifestHash`) only after the clean-14 seal is verified. Treat `artifacts/mainnet_genesis_verify_report.json`, `artifacts/MAINNET_GENESIS_VERIFIED.txt`, and `preprod:check` as the source of truth.
