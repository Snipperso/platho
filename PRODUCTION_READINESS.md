...
- Final ATH allocation is fixed as `30%` activity airdrop, `15%` initial liquidity, `10%` treasury / operations, `45%` market stability reserve, and `0%` founder allocation. The market stability reserve must stay inert until a separately audited immutable seller contract is deployed and configured for the x2..x16 tranche policy.
