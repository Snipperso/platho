...
- `web/platho-config.mjs` is pinned now and must be pinned to the verified mainnet manifest and `pwa-production` signed-bundle purpose before publishing the production PWA. `MAINNET_GENESIS_VERIFIED.txt` returns `true` and the live genesis is verified, so the config Vault, CapsuleHub, ATHMaster, UsernameRegistry, ProfileRegistry, and `deploymentManifestHash` are aligned to the verified mainnet manifest hash `8a7a5f51f0e9233f469801a6600d1e87b45c6ae52094cb297a722873dc884d13`; treat `artifacts/mainnet_genesis_verify_report.json`, `artifacts/MAINNET_GENESIS_VERIFIED.txt`, and `preprod:check` as the source of truth.
