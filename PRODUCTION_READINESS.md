...
- `web/platho-config.mjs` must be pinned to the verified mainnet manifest and `pwa-production` signed-bundle purpose before publishing the production PWA. `MAINNET_GENESIS_VERIFIED.txt` returns `true` because the clean-14 genesis (F11 hardening: BuybackBurn dead-man, ATHWallet storage top-up, MarketStabilitySeller floor) is deployed, sealed, and verified; the config is pinned now and must be pinned to the verified mainnet manifest hash `9e82eaa3f96d533a99a397cd71713736fda2e49cfdf84b495353d0bfda91f8ad`, and its Vault, CapsuleHub, ATHMaster, UsernameRegistry, ProfileRegistry, and `deploymentManifestHash` match the verified mainnet manifest confirmed at seal. Treat `artifacts/mainnet_genesis_verify_report.json`, `artifacts/MAINNET_GENESIS_VERIFIED.txt`, and `preprod:check` as the source of truth.
