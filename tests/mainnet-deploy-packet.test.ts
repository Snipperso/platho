...
function draft() {
  ...
  const code_hashes = Object.fromEntries([
    ...
    'username_registry',
    'profile_registry',
    // W1-002: lane/ticket codes FeeAccumulator binds pre-seal (appended last so existing fake hashes keep indices).
    'record_shard',
    'intro_shard',
    'public_shard',
    'airdrop_ticket',
  ...
  return {
    ...
    pre_seal_bindings: [
      ...
      ['BuybackBurn.BindBuybackTreasury', ADDR.treasuryReceiver],
...
describe('mainnet deploy packet funding semantics', () => {
  ...
  it('H-DEP-BIND-01: deploy packet carries AirdropPool/registry bindings before seal', () => {
    ...
    expect(packet.phase_2_pre_seal_bindings.map((step: any) => [step.message, step.value])).toEqual([
      ...
      ['AirdropPool.AirdropBindTreasury.treasury_address', ADDR.treasuryReceiver],
      ['FeeAccumulator.BindAirdropPool.airdrop_pool_address', ADDR.airdropPool],
      ['FeeAccumulator.BindShardCode.shard_code', source.manifest.code_hashes.record_shard],
      ['FeeAccumulator.BindIntroShardCode.intro_shard_code', source.manifest.code_hashes.intro_shard],
      ['FeeAccumulator.BindPublicShardCode.public_shard_code', source.manifest.code_hashes.public_shard],
      ['FeeAccumulator.BindTicketCode.ticket_code', source.manifest.code_hashes.airdrop_ticket],
      ['UsernameRegistry.BindOfficialAthWallet', ADDR.usernameOfficialWallet],
      ['ProfileRegistry.BindProfileOfficialAthWallet', ADDR.profileOfficialWallet],
    ]);
  });
