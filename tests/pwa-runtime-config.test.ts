...
describe('PWA runtime config guard', () => {
  ...
  it('PWA-CONFIG-01D: composers are multiline and wallet-confirmed private mode is one segment', () => {
    ...
    expect(app).toMatch(/submitted \$\{submitted\}\/\$\{total\}, confirming/);
  ...
  it('PWA-CONFIG-08: service worker precaches runtime crypto vendor modules', () => {
    ...
    expect(sw).toMatch(/\.\/platho-wallet\.mjs\?v=7/);
    expect(sw).toMatch(/\.\/pwa-contract-transactions\.mjs\?v=14/);
    expect(sw).toMatch(/\.\/vault-ton-rpc-provider\.mjs\?v=15/);
    expect(sw).toMatch(/\.\/profile-registry-ton-rpc-provider\.mjs\?v=11/);
    expect(sw).toMatch(/\.\/capsulehub-ton-rpc-provider\.mjs\?v=15/);
    expect(sw).toMatch(/\.\/ath-ton-rpc-provider\.mjs\?v=8/);
    expect(sw).toMatch(/\.\/ton-dns-provider\.mjs\?v=7/);
    expect(sw).toMatch(/\.\/username-ton-rpc-provider\.mjs\?v=13/);
    expect(sw).toMatch(/\.\/recipient-identities\.mjs\?v=2/);
    expect(sw).toMatch(/\.\/crypto\/platho-crypto\.mjs\?v=2/);
    expect(sw).toMatch(/\.\/crypto\/platho-crypto\.mjs\?v=4/);
