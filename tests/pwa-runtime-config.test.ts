...
describe('PWA runtime config guard', () => {
  ...
  it('PWA-CONFIG-01B: chat header exposes only live controls', () => {
    ...
    expect(html).toMatch(/id="appVersionLabel">v532<\/span>/);
    expect(app).toMatch(/const PLATHO_APP_RUNTIME_VERSION = 'v532'/);
  ...
  it('PWA-CONFIG-01I: opening a private chat reuses the existing dialog, and the comments toggle reads its state', () => {
    ...
    // (4) The open channel detail is FULL-SCREEN like a Private conversation: the list chrome — the
  ...
  it('PWA-CONFIG-08: service worker precaches runtime crypto vendor modules', () => {
    ...
    expect(sw).toMatch(/platho-pwa-prototype-v603/);
    expect(sw).toMatch(/\.\/styles\.css\?v=176/);
    expect(sw).toMatch(/\.\/assets\/icons\/swap-circular\.svg/);
    expect(sw).toMatch(/\.\/assets\/icons\/download\.svg/);
    expect(sw).toMatch(/\.\/app\.js\?v=532/);
