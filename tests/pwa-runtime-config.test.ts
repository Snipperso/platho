...
describe('PWA runtime config guard', () => {
  ...
  it('PWA-CONFIG-01B: chat header exposes only live controls', () => {
    ...
    expect(html).toMatch(/id="appVersionLabel">v638<\/span>/);
    expect(app).toMatch(/const PLATHO_APP_RUNTIME_VERSION = 'v638'/);
    // The app.js cache-bust query MUST track the app version (index.html's script tag here; the sw.js ASSETS
    // entry is checked in PWA-CONFIG-08), or the console shows a stale ?v= and a cached app.js can be served
    // under the old URL.
    expect(html).toMatch(/<script src="\.\/app\.js\?v=638" type="module">/);
  ...
  it('PWA-CONFIG-08: service worker precaches runtime crypto vendor modules', () => {
    ...
    expect(sw).toMatch(/platho-pwa-prototype-v709/);
    expect(sw).toMatch(/\.\/styles\.css\?v=198/);
    expect(sw).toMatch(/\.\/assets\/icons\/swap-circular\.svg/);
    expect(sw).toMatch(/\.\/assets\/icons\/download\.svg/);
    expect(sw).toMatch(/\.\/app\.js\?v=638/);
