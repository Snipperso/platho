...
describe('PWA runtime config guard', () => {
  ...
  it('PWA-CONFIG-01B: chat header exposes only live controls', () => {
    ...
    expect(html).toMatch(/id="appVersionLabel">v550<\/span>/);
    expect(app).toMatch(/const PLATHO_APP_RUNTIME_VERSION = 'v550'/);
  ...
  it('PWA-CONFIG-08: service worker precaches runtime crypto vendor modules', () => {
    ...
    expect(sw).toMatch(/platho-pwa-prototype-v621/);
    expect(sw).toMatch(/\.\/styles\.css\?v=184/);
    expect(sw).toMatch(/\.\/assets\/icons\/swap-circular\.svg/);
    expect(sw).toMatch(/\.\/assets\/icons\/download\.svg/);
    expect(sw).toMatch(/\.\/app\.js\?v=550/);
