...
describe('EVICT-INCENTIVE — eviction only happens if calling it pays', () => {
  ...
  it.each([16, 64])('an INTRO sweep of %i pays its caller more than it costs', async (n) => {
    ...
    // the fee belongs to the liquidity pool and must NOT follow the bounty out of the door
    expect((await shard.getGetView()).accrued_fee, 'the protocol fee stayed behind').toBe(BigInt(n) * 10_000_000n);
  }, 300_000);
