...
- ACK value is limited to `CAPSULEHUB_ACK_FORWARD_RESERVE = 30,000,000` nanotons (`0.030 TON`); after Vault processes that ACK under the current refund reserve, the user is credited roughly `25,800,000` nanotons in internal Vault TON balance, while later final v1 PWA surcharge above canonical required value is retained by CapsuleHub as network/storage reserve overage;
