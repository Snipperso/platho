...
## OWNER QUESTIONS
1. RECOVERY_POOL_RETENTION_SECONDS = 315360000 (~10 лет) вшивается НАВСЕГДА. Неактивность >10 лет + переустановка = permissionless-эвикция слота = необратимая потеря ВСЕХ K_root и контакт-графа. Подтвердить 10 лет или назвать иную границу.
...
retention_funding:
ПИН: `RECOVERY_POOL_RETENTION_SECONDS = 315360000` (=10 × CAPSULEHUB_INDEX_RETENTION_SECONDS 31536000). Вшивается в genesis, постфактум не тюнится.
...
2. `RECOVERY_POOL_RETENTION_SECONDS=315360000` (~10 лет). Почему 10: баланс durability↔предоплата. Длиннее → больше GRAM при старте беседы; короче → риск необратимой потери всей переписки при неактивности > retention. 10 лет покрывает реалистичную длительную неактивность и превышает практическую доступность tx-архива.
...
- [blocker] RECOVERY_POOL_RETENTION_SECONDS=315360000 (10 лет) и CAPSULEHUB_RECOVERY_ENTRY_STORAGE_ENDOWMENT вшиваются в genesis необратимо; 10-летняя рента on-chain-тела recovery — самая неопределённая storage-мера во всём дизайне (не экстраполируется от 1-год CONV).
...
КОНСТАНТЫ. `const CAPSULEHUB_RECOVERY_POOL_RETENTION_SECONDS: Int = 315360000;` (~10 лет; НЕ 1-год private). `const CAPSULEHUB_RECOVERY_ENTRY_STORAGE_ENDOWMENT: Int = <G8>;` — рента storage за 10 лет за entry (≈10× storage-часть CONV endowment; ориентир 20-30M, финализирует G8, т.к. 10× горизонт). `const ENTRY_UID_DOMAIN_VAULT_RECOVERY: Int = 0xD1190204;`.
...
RECOVERY_POOL_RETENTION_SECONDS=315360000 вшивается в genesis необратимо. Если 10 лет недостаточно (пользователь неактивен >10 лет) — recovery-капсула эвиктится, K_root потерян. Это остаточный, задокументированный BLOCKER-tail из спека; 10 лет = owner-принятая граница.
